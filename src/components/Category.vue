...
const onLoadMore = async () => {
  ...
  const categoryGoodsInfoRes = await post<CurrentCategoryGoodsInfo>('GetCurrentList', { id: unref(currentCategoryId)[0], page: currentPage.value, size: 5 })
...
watch(currentCategoryId, async () => {
  ...
  const categoryGoodsInfoRes = await post<CurrentCategoryGoodsInfo>('GetCurrentList', { id: unref(currentCategoryId)[0], page: 1, size: 5 })
  const { errno, data } = categoryGoodsInfoRes
  if (errno === 0) {
